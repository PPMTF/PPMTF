...
/*
  ...
  # [output1]: AT (N x K matrix)
  # [output2]: BT (M x K matrix)
  # [output3]: CT (M x K matrix)
  # [output4]: AV (N x K matrix)
  # [output5]: BV (M x K matrix)
  # [output6]: CV (T x K matrix)
*/
tuple<mat_t, mat_t, mat_t, mat_t, mat_t, mat_t> ReadModelParameters(const string& prefix, int K, int ItrNum){
  mat_t AT = ReadModelParameter(prefix, K, ItrNum, "AT");
  mat_t BT = ReadModelParameter(prefix, K, ItrNum, "BT");
  mat_t CT = ReadModelParameter(prefix, K, ItrNum, "CT");
  mat_t AV = ReadModelParameter(prefix, K, ItrNum, "AV");
  mat_t BV = ReadModelParameter(prefix, K, ItrNum, "BV");
  mat_t CV = ReadModelParameter(prefix, K, ItrNum, "CV");
  return forward_as_tuple(AT, BT, CT, AV, BV, CV);
}
...
/*
  ############################## Synthesize traces ##############################
  # [input1]: AT (N x K matrix)
  # [input2]: BT (M x K matrix)
  # [input3]: CT (M x K matrix)
  # [input4]: AV (N x K matrix)
  # [input5]: BV (M x K matrix)
  # [input6]: CV (T x K matrix)
  # [input7]: N -- Number of users
  # [input8]: M -- Number of POIs
  # [input9]: T -- Number of time periods
  # [input10]: poi_dic ({poi_index: category})
 */
void SynTraces(mat_t& AT,
	       mat_t& BT,
	       mat_t& CT,
	       mat_t& AV,
	       mat_t& BV,
...
//  sprintf(buf, "%s_K%d_Itr%d_A.csv", SynTraceFile.c_str(), K, ItrNum);
  ...
  for(int n = 0; n < N; ++n){
    ...
    for(int t = 0; t < T; ++t){
      mat_t ad = AV.row(n).array() * CV.row(t).array();
      for(int i = 0; i < M; ++i){
	// Elements in a sampled visit tensor --> time_poi_dist
	time_poi_dist(t, i) = (ad.array() * BV.row(i).array()).sum();
    ...
    for(int i = 0; i < M; ++i){
      mat_t ab = AT.row(n).array() * BT.row(i).array();
      // Elements in a sampled transition tensor (assign TransDelta for a small transition count) --> prop_mat
      for(int j = 0; j < M; ++j){
	// prop_mat[i,j] = max(np.sum(ab * CT[j, :]), TransDelta)
	prop_mat(i,j) = max((ab.array() * CT.row(j).array()).sum(), TransDelta);
...
int main(int argc, char *argv[]){
  if(argc < 3){
      printf("Usage: SynData_PPITF [Dataset] [City] ([TraceNum (default:10)] [Alp (default:200)] [MaxNumTrans (default:100)] [MaxNumVisit (default:100)] [ItrNum (default:100)])\n");
      return -1;
  } 
...
//  const int TimInsNum = 1;
  ...
  // Read model parameters
  mat_t AT, BT, CT, AV, BV, CV;
  tie(AT, BT, CT, AV, BV, CV) = ReadModelParameters(ModelParameterFile, K, ItrNum);

  // Synthesize traces
  SynTraces(AT, BT, CT, AV, BV, CV, N, M, T, poi_dic,

...
tuple<mat_t, mat_t, mat_t, mat_t, mat_t, mat_t> ReadModelParameters(const string& ModelParameterFile,
						      int K,
						      int ItrNum){
  mat_t A = ReadModelParameter(ModelParameterFile, K, ItrNum, "AT");
  mat_t B = ReadModelParameter(ModelParameterFile, K, ItrNum, "BT");
  mat_t C = ReadModelParameter(ModelParameterFile, K, ItrNum, "CT");
  mat_t AV = ReadModelParameter(ModelParameterFile, K, ItrNum, "AV");
  mat_t BV = ReadModelParameter(ModelParameterFile, K, ItrNum, "BV");
  mat_t CV = ReadModelParameter(ModelParameterFile, K, ItrNum, "CV");
  return forward_as_tuple(A,B,C,AV,BV,CV);
}
...
int main(int argc, char* argv[]){
  ...
  //string OutDir = DataDir + "PPITF_" + City + "_alp" + AlpStr + "_mnt" + i2s(MaxNumTrans) + "_mnv" + i2s(MaxNumVisit) + "_py/";
  string OutDir = DataDir + "PPITF_" + City + "_alp" + AlpStr + "_mnt" + i2s(MaxNumTrans) + "_mnv" + i2s(MaxNumVisit) + "/";
...
//  seeding(1);
  ...
  rep(n, N){
    for(const auto& it : RT1_observed[n]){
      ...
      double rhs = abs(val - (AT.row(n).array() * BT.row(i).array() * CT.row(j).array() ).sum());
      if(val > 0.0){
    ...
    for(const auto& it : RV1_observed[n]){
      ...
      int j = p.second;
      double val = it.second;
      double rhs = abs(val - (AV.row(n).array() * BV.row(i).array() * CV.row(j).array() ).sum());
      if(val > 0.0){
